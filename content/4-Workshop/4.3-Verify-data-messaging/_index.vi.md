...
1. **Amazon RDS (MySQL):** Kiểm tra xem các cơ sở dữ liệu `auth`, `event`, `ticket`,... đã được khởi tạo thành công chưa.
   ...
   ![Tìm kiếm dịch vụ RDS](/images/4.3-Verify-data-messaging/image-1.png)
   
   - Nhấp vào mục **Databases** ở thanh công cụ bên trái để xem danh sách.
   
   ![Chọn cơ sở dữ liệu](/images/4.3-Verify-data-messaging/image.png)
   
   - Xác nhận các database đã được tạo thành công với trạng thái **Available**.
   
   ![Xác nhận RDS tạo thành công](/images/4.3-Verify-data-messaging/image-2.png)
...
2. **Amazon ElastiCache (Redis):** Xác minh Redis endpoint đã hình thành.
   ...
   ![Tìm kiếm dịch vụ Redis](/images/4.3-Verify-data-messaging/image-3.png)
   
   - Chọn mục **Redis clusters** ở thanh công cụ bên.
   
   ![Chọn danh sách Redis clusters](/images/4.3-Verify-data-messaging/image-4.png)
   
   - Xác nhận cluster Redis đã được tạo thành công.
   
   ![Xác nhận Redis tạo thành công](/images/4.3-Verify-data-messaging/image-5.png)
...
3. **Amazon SQS:** Đảm bảo hàng đợi SQS đã sẵn sàng nhận tin nhắn sự kiện.
   ...
   ![Tìm kiếm dịch vụ SQS](/images/4.3-Verify-data-messaging/image-6.png)
   
   - Xác nhận các hàng đợi (queues) SQS đã được liệt kê đầy đủ trên giao diện.
   
   ![Xác nhận SQS tạo thành công](/images/4.3-Verify-data-messaging/image-7.png)
